...
RTC_DATA_ATTR int MyGSRQLockWatchFaceId; // Watchface ID #.
// Approx. 3035 bytes

class GSRWatchFaceQLock : public WatchyGSR
{
public:
  GSRWatchFaceQLock() : WatchyGSR() { initAddOn(this); } // *** DO NOT EDIT THE CONTENTS OF THIS FUNCTION ***

  QLock QLockWatchFace;

  void RegisterWatchFaces()
  { // Add WatchStyles here (this is done pre-boot, post init)
    MyGSRQLockWatchFaceId = AddWatchStyle("QLock", this);
  };

  // void InsertInitWatchStyle(uint8_t StyleID){
  //   if (StyleID == GSRQLockWatchFaceId){ // LCD
  //       Design.Menu.Top = 56;
  ...
  void InsertDrawWatchStyle(uint8_t StyleID)
  {
    uint8_t X, Y;
    uint16_t A;
    if (StyleID == MyGSRQLockWatchFaceId)
    {
      if (SafeToDraw())
      {
        QLockWatchFace.drawWatchFace();
      }
    }
  };
};

GSRWatchFaceQLock MyGSRWatchFaceQLock;
